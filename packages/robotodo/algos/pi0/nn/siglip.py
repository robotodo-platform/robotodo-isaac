...
class _Module(nn.Module):
    ...
    def init(self, image, *, train=False):
        # TODO NOTE image: batch, height, width, channels
        image = jnp.asarray(image)
        if any(s % p != 0 for s, p in zip(image.shape[1:3], self.patch_size)):
            raise ValueError(
                f"Image shape ({image.shape}) must be divisible by "
                f"the patch size ({self.patch_size}) in height and width."
            )
        self(image, train=train)

    @nn.compact
    def __call__(self, image, *, train=False):
        # TODO NOTE image must be minimum of patch_size 
